...
export const InputTexto: React.FC<{ onChange?: (valor: string) => void;   title?: string; }> = ({  onChange, title }) => {
...
export const InputNumero: React.FC<{
  ...
  return (
    <div>
      ...
      <div>
        <button type="button"  onClick={handleDecrement} className="px-6 bg-gray-200 rounded">−</button>
        <button type="button"  onClick={handleIncrement} className="m-2 px-6 bg-gray-200 rounded">+</button>
      </div>
    </div>
  );
};
...
export const InputEndereco: React.FC<{enderecoParam?: string;   onChange?: (valor: string) => void; detalhes?:(produtosList: any[]) => void; title?: string }> = ({enderecoParam = '', onChange, detalhes, title}) => {
